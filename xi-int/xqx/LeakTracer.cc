...
{
	static ssize_t (*readfn)(int, void*, size_t);
//	static sszie_t * (* readfn)(int, void*, size_t) = NULL;
	ssize_t ret;
	readfn = (ssize_t (*)(int, void*, size_t)) dlsym(RTLD_NEXT, "read");
	if( readfn == NULL) {
		printf("read get error\n");
		return 0;
	}
	ret = readfn(fd, buf, nbytes);
	printf("read called ,buf = %x, size = %d, ret = %d\n", buf, nbytes, ret);
        return ret ;
}
